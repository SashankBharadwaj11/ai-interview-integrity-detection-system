...
class ReportGenerator:
    def __init__(self, config):
        ...
        # Support offline alert type names as well
        self.severity_map.update({
            'FACE_MISSING': self.severity_map.get('FACE_DISAPPEARED', 1),
            'MOUTH_MOVEMENT': self.severity_map.get('MOUTH_MOVING', 3),
            'MULTI_FACE': self.severity_map.get('MULTIPLE_FACES', 4),
        })
    ...
    def generate_offline_report_from_session_log(
        ...
        """
        Generate a report for an OFFLINE prerecorded session using the JSON
        log produced by OfflineExamAnalyzer.
        """
        try:
            ...
            # Fake an absolute time base so the timeline plotting code works.
            base = datetime.now()

            violations = []
            for a in alerts:
                # a['timestamp'] is seconds since start; convert to datetime
            ...
            # Minimal "student_info" for offline sessions
            student_info = {
                "id": session_id,
                "name": f"Offline session {session_id}",
                "email": "",
                "exam": self.config.get("default_exam_name", "Offline Exam"),
            ...
            self.logger.error(f"Failed to generate offline report: {e}")
            return None
